        ...
        list.sort(new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                if (compare1 != 0)
                    ...
                    Date data1 = (Date) stock1.get("date");
                    Date data2 = (Date) stock2.get("date");
                    SimpleDateFormat df = new SimpleDateFormat("yyyyMMdd");
                    int compare2 = df.format(data2).compareTo(df.format(data1));
                    if (compare2 != 0)
                        return compare2;
                    else {
                        Double arrive1;
                        Double arrive2;
                        if (stock1.get("last") != null)
                            arrive1 = (double) stock1.get("last") - (double) stock1.get("open");
                        else
                            arrive1 = (double) stock1.get("change");
                        if (stock2.get("last") != null)
                            arrive2 = (double) stock2.get("last") - (double) stock2.get("open");
                        else
                            arrive2 = (double) stock2.get("change");
                        return arrive2.compareTo(arrive1);
                    }
                }
            }
        });
